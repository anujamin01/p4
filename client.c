...
{
    sprintf(logBuffer, "Attemping to connect to %s:%d", hostname, port);
    INFO();
    int rc = MFS_Init(hostname, port);
    if (rc != 0)
    {
        sprintf("MFS_Init failed for %s:%d", hostname, port);
        ERR();
    }
    return rc;
}
...
int _traverseToDirectory(char *path)
{
    ...
    while (dirname != NULL)
    {
        ...
        sprintf(logBuffer, "looking up child entry %s in parent directory (inode=%d)", dirname, dirInode);
        VERBOSE();
        dirInode = MFS_Lookup(dirInode, dirname);
        sprintf(logBuffer, "Found child entry with inode number %d", dirInode);
        VERBOSE();

        if (dirInode == -1)
        {
            sprintf(logBuffer, "Unable to enter %s", dirname);
            ERR();
            exit(1);
        };
        dirname = strtok(NULL, "/");
...
int perform_ls(char *path)
{
    ...
    if (rc == -1)
    {
        sprintf(logBuffer, "Unable to call MFS_Stat on dir (inum=%d)", dirInode);
        ERR();
    }
    if (stat.type != UFS_DIRECTORY)
    {
        sprintf(logBuffer, "The inode (%d) received for %s is not of directory type", dirInode, path);
        ERR();
    }
    ...
    sprintf(logBuffer, "Attempting to read %d children of %s", nentries, path);
    VERBOSE();
    ...
    while (offset < sz)
    {
        ...
        if (toRead > MFS_RW_BUFFER_SIZE)
            toRead = MFS_RW_BUFFER_SIZE;
        rc = MFS_Read(dirInode, (char*)entries, offset, toRead); // changed this
        if (rc == -1)
        {
            sprintf(logBuffer, "MFS_Read failed");
            ERR();
        }
        offset += toRead;
    }

    sprintf(logBuffer, "Fetched %d children. Here they are!", nentries);
    INFO();
...
int perform_insert(const char *fromPath, char *toPath)
{
    ...
    if (toCopyFd == -1)
    {
        sprintf(logBuffer, "Unable to open provided file %s", fromPath);
        ERR();
    }
    ...
    sprintf(logBuffer, "Trying to create new file %s in %s", fileName, dirPath);
    VERBOSE();

    int rc = MFS_Creat(dirInode, UFS_REGULAR_FILE, fileName);
    if (rc == -1)
    {
        sprintf(logBuffer, "Unable to create new file %s in %s", fileName, dirPath);
        ERR();
    }
    ...
    if (newInode == -1)
    {
        sprintf(logBuffer, "Unable to fetch newly created inode number even though MFS_Creat was successful");
        ERR();
    }

    sprintf(logBuffer, "Created new file with inode number %d", newInode);
    INFO();
    ...
    while (readBytes > 0)
    {
        sprintf(logBuffer, "about to write %d bytes ", readBytes);
        VERBOSE();
        ...
        if (rc == -1)
        {
            sprintf(logBuffer, "MFS_Write failed");
            ERR();
        }
        sprintf(logBuffer, "Written %d bytes successfully", readBytes);
        VERBOSE();
        readBytes = read(toCopyFd, buffer, MFS_RW_BUFFER_SIZE);
    }
    if (readBytes == -1)
    {
        sprintf(logBuffer, "Error while reading input file");
        ERR();
    }

    sprintf(logBuffer, "Completed all write operations. Written a total of %d bytes", offset);
    INFO();
...
int perform_cat(char *path)
{
    ...
    if (fileInode == -1)
    {
        sprintf(logBuffer, "Unable to lookup file %s in directory (inum=%d)", fileName, dirInode);
        ERR();
    }

    sprintf(logBuffer, "Trying to determine filesize");
    VERBOSE();
    ...
    if (rc == -1)
    {
        sprintf(logBuffer, "Unable to determine filesize. Stat failed for inum=%d", fileInode);
        ERR();
    }
    ...
    sprintf(logBuffer, "Filesize=%d. Starting read", sz);
    INFO();
    ...
    {
        int count = sz - offset;
        if (count > MFS_RW_BUFFER_SIZE)
            count = MFS_RW_BUFFER_SIZE;

        sprintf(logBuffer, "Trying to read %d bytes from offset %d foi inum=%d", count, offset, fileInode);
        VERBOSE();
        int rc = MFS_Read(fileInode, output + offset, offset, count);
        if (rc == -1)
        {
            sprintf(logBuffer, "MFS_Read failed for inum=%d offset=%d count=%d", fileInode, offset, count);
            ERR();
        }
...
int perform_mkdir(char *path)
{
    ...
    int dirInode = 0;
    while (dirname != NULL)
    { // assume root directory already exists. Creating further ones.
        if (strcmp(dirname, "") == 0)
        {
            dirname = strtok(NULL, "/");
            continue; // to handle // and trailing /
        ...
        sprintf(logBuffer, "calling MFS_Creat for %s in parent directory (inode=%d)", dirname, dirInode);
        VERBOSE();
        int rc = MFS_Creat(dirInode, UFS_DIRECTORY, dirname);
        if (rc == -1)
        {
            sprintf(logBuffer, "Unable to create directory %s", dirname);
            ERR();
        }
        ...
        if (newInode == -1)
        {
            sprintf(logBuffer, "Unable to fetch newly created directory inode even though MFS_Creat was successful");
            ERR();
        }
    ...
    sprintf(logBuffer, "mkdir completed successfully");
    INFO();
    free(path);
    return 0;
}
