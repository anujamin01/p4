...
class MaxFileTest(MfsTest):
   ...
   def run(self):
      ...
      for i in range(MAX_FILE_BLOCKS):
         self.write(inum, buf[i], i, MFS_BLOCK_SIZE * MAX_FILE_BLOCKS)

      for i in range(MAX_FILE_BLOCKS):
         self.read_and_check(inum, i, buf[i], MFS_BLOCK_SIZE * MAX_FILE_BLOCKS)
...
class MaxFile2Test(MfsTest):
   ...
   def run(self):
      ...
      for i in range(MAX_FILE_BLOCKS):
         self.write(inum, buf[i], i, MFS_BLOCK_SIZE)
      i = MAX_FILE_BLOCKS
      r = self.libmfs.MFS_Write(inum, byref(buf[i]), i, 1)
      if r != -1:
         raise Failure("MFS_Write should fail on inalid block number")

      for i in range(MAX_FILE_BLOCKS):
         self.read_and_check(inum, i, buf[i], MFS_BLOCK_SIZE)
      i = MAX_FILE_BLOCKS
      r = self.libmfs.MFS_Read(inum, byref(buf[i]), i, 1)
      if r != -1:
